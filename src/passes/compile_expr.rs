...
// use crate::pos::Sp;
// use crate::instr::{LowLevelStmt, Instr};
...
//
//
// /// Take statements with complicated expressions and expand them into statements with simple expressions.
// pub fn compile_exprs(original_stmts: &[LowLevelStmt]) -> Result<Vec<LowLevelStmt>, CompileError> {
//     // holds assignments we still need to look at
//     let mut assignment_stack: Vec<(VarId, ast::Expr)> = vec![];
//
//     let opcode_intrinsics: HashMap<_, _> = format.intrinsic_opcode_pairs().into_iter().collect();
//
//     let mut out = vec![];
//
//     for stmt in original_stmts.iter().cloned() {
//         let mut instr = match stmt {
...
//
//                 out.push(LowLevelStmt::Instr(Instr {
//                     time: stmt.time,
